...
class CPU():
    ...
    def absolute(self, bytesToRead, cycles, func):
        valueAddress = (self.memory.read(self.pc+2) << 8) | (self.memory.read(self.pc+1))
    ...
    def CPY(self, value, addr=None):
        ...
        self.carryFlag = (self.regY >= value)
        self.zeroFlag = (self.regY == value)
        self.updateNegativeFlag(subtractResult)
    ...
    def ADC(self, value, addr=None):
        ...
        self.updateZeroFlag(result)
        self.overflowFlag = (result ^ self.regA) & (result ^ value) & 0x80
        self.updateNegativeFlag(result)
    ...
    def SBC(self, value, addr=None):
        ...
        self.updateZeroFlag(result)
        self.overflowFlag = (result ^ self.regA) & (result ^ invertedValue) & 0x80
        self.updateNegativeFlag(result)
